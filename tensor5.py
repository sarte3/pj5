...
# h=w1x1+w2x2+b -> x X w(행렬곱) + b
# water.csv 파일은 총 정수기 대여 대수(전월),
# 10년 이상 노후 정수기 대여 대수(전월), AS시간(당월)에 대한 데이터이다.
# 주변의 신규 아파트가 동시 입주함에 따라 가입자수가 늘어 다음달의 AS시간을 예측하고 이에 따라 신규인력을 채용하고자 한다
...
#     print('b=', sess.run(b))
#
# # 월말 최종 대여수를 보니 총 대여수가 300,000대, 그 중 10년 이상 노후 정수기 대수가 70,000대로 집계되었다.
# # 다음 달의 AS시간을 예측하고 그에 따라 필요한 AS기사의 인원 수를 예측하시오
...
onehot2 = tf.reshape(onehot, [-1,7]) # 차원 축소
...
with tf.Session() as sess:
    ...
    # print('학습전 정확도 = ', sess.run(tf.reduce_mean(tf.cast(tf.equal(tf.argmax(h, 1), tf.argmax(onehot2, 1)), tf.float32)), feed_dict={x: xdata, y: ydata}))
    for i in range(2001):
        sess.run(train, feed_dict={x: xdata, y: ydata})
        if i % 100 == 0:
            print(sess.run(cost, feed_dict={x: xdata, y: ydata}))
    # 평가하기
    correct = tf.equal(tf.argmax(h, 1), tf.argmax(onehot2, 1))
