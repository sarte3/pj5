...
# train = tf.train.GradientDescentOptimizer(0.00053).minimize(cost)
...
# data.kma.go.kr
import pandas as pd
# data = pd.read_csv('data/temp.csv', engine='python', parse_dates=['일시'])
# # print(data)
# # print(data.info())
...
with tf.Session() as sess:
    ...
    for i in range(5001):
        ...
        if i % 1000 == 0:
            print(sess.run(cost, feed_dict={x: trainx, y: trainy}))
    # 검증하기
    result = sess.run(h, feed_dict={x: testx})
    plt.plot(testy)
    plt.plot(result)
    plt.show()
